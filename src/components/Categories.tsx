...
export default function Categories() {
    ...
    useEffect(() => {
        const storedItemsString = localStorage.getItem('list');
        if (storedItemsString) {
          const storedItems: Item[] = JSON.parse(storedItemsString);
          setItems(storedItems);
        } else {
          setItems([{ id: 1, name: 'Item 1', subItems: [] }]);
        }
      }, []);
        ...
        <div className='wrapper'>
            ...
            <div className="wrapper__datas">
                ...
                {items?.map((item, index) => (
                    <div key={item.id} className="wrapper__row">
                        <Link to={`/category/${item.id}`}>{item.name}</Link>
                        <div className="wrapper__row__actions">
                            <svg onClick={() => deleteItem(item)} width="20" height="22" viewBox="0 0 20 22" fill="none" xmlns="http://www.w3.org/2000/svg">
                            ...
                            <svg onClick={() => toggleEditOpen(item)} width="22" height="21" viewBox="0 0 22 21" fill="none" xmlns="http://www.w3.org/2000/svg">
